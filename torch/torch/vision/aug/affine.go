...
func (ra *RandomAffine) getParams(imageSize []int64) (float64, []int64, float64, []float64) {
	angleTs := ts.MustEmpty([]int64{1}, torch.Float, torch.CPU)
	...
	if ra.translate != nil {
		...
		dx := ts.MustEmpty([]int64{1}, torch.Float, torch.CPU)
		dx.MustUniform_(-maxDX, maxDX)
		tx := dx.Float64Values()[0]
		dx.MustDrop()

		dy := ts.MustEmpty([]int64{1}, torch.Float, torch.CPU)
	...
	scale := 1.0
	if ra.scale != nil {
		scaleTs := ts.MustEmpty([]int64{1}, torch.Float, torch.CPU)
	...
	if ra.shear != nil {
		shearXTs := ts.MustEmpty([]int64{1}, torch.Float, torch.CPU)
		shearXTs.MustUniform_(ra.shear[0], ra.shear[1])
		shearX = shearXTs.Float64Values()[0]
		shearXTs.MustDrop()

		if len(ra.shear) == 4 {
			shearYTs := ts.MustEmpty([]int64{1}, torch.Float, torch.CPU)
