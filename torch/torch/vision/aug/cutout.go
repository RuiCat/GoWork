...
func (rc *RandomCutout) cutoutParams(x *ts.Tensor) (int64, int64, int64, int64, *ts.Tensor) {
	...
	for i := 0; i < 10; i++ {
		...
		ratioTs := ts.MustEmpty([]int64{1}, torch.Float, torch.CPU)
		...
		iTs := ts.MustRandint(imgH-h+1, []int64{1}, torch.Int64, torch.CPU)
		i := iTs.Int64Values()[0]
		iTs.MustDrop()
		// j = torch.randint(0, img_w - w + 1, size=(1, )).item()
		jTs := ts.MustRandint(imgW-w+1, []int64{1}, torch.Int64, torch.CPU)
		j := jTs.Int64Values()[0]
		jTs.MustDrop()
		return i, j, h, w, v
	}
...
func (rc *RandomCutout) Forward(img *ts.Tensor) *ts.Tensor {
	...
	randTs := ts.MustRandn([]int64{1}, torch.Float, torch.CPU)
