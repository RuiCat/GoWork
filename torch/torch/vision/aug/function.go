...
func gaussianKernel1D(ks int64, sigma float64, dtype torch.DType, device torch.Device) *ts.Tensor {
	ksHalf := (ks - 1) / 2
...
func gaussianKernel2D(ks []int64, sigma []float64, dtype torch.DType, device torch.Device) *ts.Tensor {
...
func containsDType(dtype torch.DType, dtypes []torch.DType) bool {
...
func castSqueezeIn(x *ts.Tensor, reqDtypes []torch.DType) (*ts.Tensor, bool, bool, torch.DType) {
	needSqueeze := false
	xdim := x.MustSize()
	var img *ts.Tensor
	if len(xdim) < 4 {
...
func castSqueezeOut(x *ts.Tensor, needCast, needSqueeze bool, outDType torch.DType) *ts.Tensor {
	var (
	...
	case true:
		...
		if containsDType(outDType, []torch.DType{torch.Uint8, torch.Int8, torch.Int16, torch.Int, torch.Int64}) {
...
func gaussianBlur(x *ts.Tensor, ks []int64, sigma []float64) *ts.Tensor {
	// dtype := torch.Float
	dtype := x.DType()
	if x.DType() == torch.Float || x.DType() == torch.Double {
		dtype = x.DType()
	}
	device := x.MustDevice()
	...
	img, needCast, needSqueeze, outDType := castSqueezeIn(x, []torch.DType{kdtype})
...
func randVal(from, to float64) float64 {
	v := ts.MustEmpty([]int64{1}, torch.Float, torch.CPU)
...
func adjustContrast(x *ts.Tensor, contrast float64) *ts.Tensor {
	...
	mean := grayTs.MustMeanDim([]int64{-3, -2, -1}, true, torch.Float, true).MustTotype(x.DType(), true)
...
func hsv2RGB(x *ts.Tensor) *ts.Tensor {
	...
	i = i.MustTotype(torch.Int, true)
	//i = i % 6
	iremainder := i.MustRemainder(ts.IntScalar(6), true) // delete i
	// torch.arange(6, device=i.device).view(-1, 1, 1)
	x4 := ts.MustArange(ts.FloatScalar(6), torch.Float, iremainder.MustDevice()).MustView([]int64{-1, 1, 1}, true)
...
func adjustHue(x *ts.Tensor, hue float64) *ts.Tensor {
	...
	imgFl := x.MustTotype(torch.Float, false).MustDivScalar(ts.FloatScalar(255.0), true)
...
func centerCrop(x *ts.Tensor, size []int64) *ts.Tensor {
	...
	if cropW > imgW || cropH > imgH {
		...
		// floatX := x.MustTotype(torch.Float, false)
...
func rotImg(x *ts.Tensor, theta float64, dtype torch.DType) (*ts.Tensor, error) {
...
func applyGridTransform(x, gridInput *ts.Tensor, mode string, fillValue []float64) *ts.Tensor {
	...
	img, needCast, needSqueeze, outDtype := castSqueezeIn(x, []torch.DType{dtype})
...
func perspectiveCoeff(startPoints, endPoints [][]int64) []float64 {
	...
	aMat := ts.MustZeros([]int64{size, 8}, torch.Float, torch.CPU)
	...
	// bMat := ts.MustOfSlice(startPoints).MustTotype(torch.Float, true).MustView([]int64{8}, true)
	bMat := ts.MustOfSlice(startData).MustTotype(torch.Float, true).MustView([]int64{8}, true)
...
func perspectiveGrid(coef []float64, ow, oh int64, dtype torch.DType, device torch.Device) *ts.Tensor {
...
func perspective(x *ts.Tensor, startPoints, endPoints [][]int64, mode string, fillValue []float64) *ts.Tensor {
	...
	device := x.MustDevice()
	grid := perspectiveGrid(coef, ow, oh, torch.Float, device)
...
func autocontrast(img *ts.Tensor) *ts.Tensor {
	...
	eqIdx := minTs.MustEqTensor(maxTs, false).MustSqueezeDim(-1, true).MustSqueezeDim(-1, true).MustTotype(torch.Int64, true)
...
func blurredDegenerateImage(img *ts.Tensor) *ts.Tensor {
	...
	resTmp, needCast, needSqueeze, outDtype := castSqueezeIn(img, []torch.DType{kdtype})
...
func scaleChannel(imgChan *ts.Tensor) *ts.Tensor {
	...
	if device == torch.CPU {
		...
		histo = imgChan.MustTotype(torch.Float, false).MustHistc(256, true)
	}
	...
	step := nonzeroHisto.MustNarrow(0, 0, histoLen-1, true).MustSum(torch.Float, true).MustFloorDivideScalar(ts.FloatScalar(255.0), true)
	...
	flattenImg := imgChan.MustFlatten(0, -1, false).MustTotype(torch.Int64, true)
	out := lut.MustIndexSelect(0, flattenImg, true).MustReshapeAs(imgChan, true).MustTotype(torch.Uint8, true)
	flattenImg.MustDrop()
...
func normalize(img *ts.Tensor, mean, std []float64) *ts.Tensor {
	...
	meanTs := ts.MustOfSlice(mean).MustTotype(torch.Float, true).MustTo(device, true)
	stdTs := ts.MustOfSlice(std).MustTotype(torch.Float, true).MustTo(device, true)
...
func Float2ByteImage(x *ts.Tensor) *ts.Tensor {
	dtype := x.DType()
	if dtype != torch.Float && dtype != torch.Double {
		err := fmt.Errorf("Input tensor is not float dtype (%v)", dtype)
		panic(err)
	}

	return x.MustMulScalar(ts.IntScalar(255), false).MustTotype(torch.Uint8, true)
}
