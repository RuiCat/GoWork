...
func readFile(filename string) (imagesTs *ts.Tensor, labelsTs *ts.Tensor) {
	...
	images := ts.MustZeros([]int64{samplesPerFile, cfC, cfH, cfW}, torch.Float, torch.CPU)
	labels := ts.MustZeros([]int64{samplesPerFile}, torch.Int64, torch.CPU)
	...
	for idx := 0; idx < int(samplesPerFile); idx++ {
		...
		tmp3 := tmp2.MustTo(torch.CPU, true)
	...
	tmp1 := images.MustTotype(torch.Float, true)
