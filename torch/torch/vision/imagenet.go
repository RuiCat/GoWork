...
func (in *ImageNet) Normalize(tensor *ts.Tensor) (*ts.Tensor, error) {
	...
	res, err := tensor.Totype(torch.Float, false)
	if err != nil {
		return nil, err
	}
...
func (in *ImageNet) UnNormalize(tensor *ts.Tensor) (*ts.Tensor, error) {
	...
	res, err := resClamp.Totype(torch.Uint8, true)
	if err != nil {
		return nil, err
	}
...
func (in *ImageNet) LoadFromDir(path string) (*Dataset, error) {
	...
	for labelIdx, labelDir := range classes {
		...
		trainLabelOnes := ts.MustOnes([]int64{ntrainTs}, torch.Int64, torch.CPU)
		...
		testLabelOnes := ts.MustOnes([]int64{ntestTs}, torch.Int64, torch.CPU)
