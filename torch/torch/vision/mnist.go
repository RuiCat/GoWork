...
func readLabels(filename string) *ts.Tensor {
	...
	return labelsTs.MustTotype(torch.Int64, true)
}
...
func readImages(filename string) *ts.Tensor {
	...
	return imagesTs.MustView([]int64{int64(samples), int64(rows * cols)}, true).MustTotype(torch.Float, true).MustDivScalar(ts.FloatScalar(255.0), true)
}
