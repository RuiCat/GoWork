...
func decodeSize(ptr unsafe.Pointer, nsize uint64) []int64 {
	dtype := torch.Int64 // tensor size dtype = int64
...
func WithDType(v torch.DType) TensorOpt {
	return func(o *TensorOptions) {
		o.DType = v
	}
}
...
func OfSlice(data interface{}, opts ...TensorOpt) (*Tensor, error) {
	...
	dtype, err := torch.GoKind2DType(elementKind, torch.HalfDTypePref(o.DType), torch.WithQuantized(o.Quantized))
	if err != nil {
		return nil, err
	}
...
func OfDataSize(data []byte, shape []int64, dtype torch.DType, opts ...TensorOpt) (*Tensor, error) {
	o := DefaultTensorOptions()
	for _, opt := range opts {
		opt(o)
	}
...
// or panic if error
func MustOfDataSize(data []byte, size []int64, dtype torch.DType, opts ...TensorOpt) *Tensor {
...
func NewTensorFromData(data interface{}, shape []int64, opts ...TensorOpt) (*Tensor, error) {
	...
	dtype, err := torch.DTypeFromData(data)
	if err != nil {
		return nil, err
	}
...
func (ts *Tensor) DType() torch.DType {
	cint := lib.AtScalarType(ts.ctensor)

	return torch.CKind2DType(cint)
}

func (ts *Tensor) Device() (torch.Device, error) {
	var (
		retVal torch.Device
		err    error
	)
...
func (ts *Tensor) MustDevice() torch.Device {
...
//
// The file format is the same as the one used by the PyTorch C++ API.
func LoadMultiWithDevice(path string, device torch.Device) ([]NamedTensor, error) {
	var data lib.LoadData
	dataPtr := lib.PStore.Set(&data)
...
// MustLoadMulti loads some named tensors from a file. It will panic if error
func MustLoadMultiWithDevice(path string, device torch.Device) []NamedTensor {
...
func (ts *Tensor) Float64Values(delOpt ...bool) []float64 {
	...
	float64Ts := ts.MustTotype(torch.Double, false)
...
func (ts *Tensor) Int64Values(delOpt ...bool) []int64 {
	...
	int64Ts := ts.MustTotype(torch.Int64, false)
...
func (ts *Tensor) Onehot(labels int64) *Tensor {
	...
	inputTs := unsqueezeTs.MustTotype(torch.Int64, true)

	zerosTs := MustZeros(dims, torch.Float, torch.CPU)
