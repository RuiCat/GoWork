...
func ModuleLoadOnDevice(path string, device torch.Device) (*CModule, error) {
...
// This function loads the model directly on the specified device,
// which means it also allows loading a GPU model on the CPU without having a CUDA enabled GPU.
func ModuleLoadDataOnDevice(stream io.Reader, device torch.Device) (*CModule, error) {
	buf := new(bytes.Buffer)
	buf.ReadFrom(stream)
...
func (cm *CModule) To(device torch.Device, kind torch.DType, nonBlocking bool) {
