...
func (it *Iter2) Shuffle() {
	index := MustRandperm(it.totalSize, torch.Int64, torch.CPU)
...
func (it *Iter2) ToDevice(device torch.Device) *Iter2 {
	it.device = device
	return it
}
...
func (td *TextData) IterShuffle(seqLen int64, batchSize int64) *TextDataIter {
	...
	return &TextDataIter{
		...
		BatchIndex: 0,
		BatchSize:  batchSize,
		Indexes:    MustRandperm(indexesLen, torch.Int64, torch.CPU),
