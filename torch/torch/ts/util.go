...
func DataCheck(data interface{}) (dtype torch.DType, n int, err error) {
	return dataCheck(reflect.ValueOf(data).Interface(), 0)
}

// NOTE: 0 is reflect.Kind() of Invalid
// See: https://golang.org/pkg/reflect/#Kind
func dataCheck(data interface{}, count int) (dtype torch.DType, n int, err error) {
	...
	total += 1
	dtype, err = torch.GoKind2DType(v.Kind())
	if err != nil {
