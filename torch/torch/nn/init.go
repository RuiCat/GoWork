...
type Init interface {
	...
	InitTensor(dims []int64, device torch.Device, dtypeOpt ...torch.DType) (retVal *ts.Tensor)
...
func (c constInit) InitTensor(dims []int64, device torch.Device, dtypeOpt ...torch.DType) (retVal *ts.Tensor) {
	dtype := torch.DefaultDType
	if len(dtypeOpt) > 0 {
		dtype = dtypeOpt[0]
	}
...
func (r randnInit) InitTensor(dims []int64, device torch.Device, dtypeOpt ...torch.DType) (retVal *ts.Tensor) {
	dtype := torch.DefaultDType
	if len(dtypeOpt) > 0 {
		dtype = dtypeOpt[0]
	}
...
func (u uniformInit) InitTensor(dims []int64, device torch.Device, dtypeOpt ...torch.DType) (retVal *ts.Tensor) {
	dtype := torch.DefaultDType
	if len(dtypeOpt) > 0 {
		dtype = dtypeOpt[0]
	}
...
func (k *kaimingUniformInit) InitTensor(dims []int64, device torch.Device, dtypeOpt ...torch.DType) (retVal *ts.Tensor) {
	dtype := torch.DefaultDType
	if len(dtypeOpt) > 0 {
		dtype = dtypeOpt[0]
	}
...
func (gl glorotNInit) InitTensor(dims []int64, device torch.Device) (retVal *ts.Tensor) {
	// TODO: implement
