...
func NewVarStore(device torch.Device) *VarStore {
	return &VarStore{
		device: device,
...
func (vs *VarStore) Device() torch.Device {
	return vs.device
}
...
func (vs *VarStore) ToDType(dtype torch.DType) {
...
func (vs *VarStore) ToDevice(device torch.Device) {
	p := vs.Root()
	p.ToDevice(device)
}
...
func (p *Path) Device() torch.Device {
...
// "torch.Float", "torch.Half", "torch.BFloat16", "torch.Double".
func (p *Path) ToDType(dtype torch.DType) {
	p.varstore.Lock()
	defer p.varstore.Unlock()
	path := strings.Join(p.path, SEP)
	for name, v := range p.varstore.vars {
...
func (p *Path) toFloat(dtype torch.DType) {
	p.varstore.Lock()
	defer p.varstore.Unlock()
	path := strings.Join(p.path, SEP)
	for name, v := range p.varstore.vars {
...
func (p *Path) ToFloat(floatDTypeOpt ...torch.DType) {
	dtype := torch.Float
	if len(floatDTypeOpt) > 0 {
		dt := floatDTypeOpt[0]
		if !torch.IsFloatDType(dt) {
			// Ingore the option
			if torch.Debug {
