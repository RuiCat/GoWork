...
func (re *RobertaEmbeddings) createPositionIdsFromInputIds(x *ts.Tensor) *ts.Tensor {
	mask := x.MustNe(ts.IntScalar(re.paddingIndex), false).MustTotype(torch.Int64, true)
	cumSum := mask.MustCumsum(1, torch.Int64, false)
...
func (re *RobertaEmbeddings) createPositionIdsFromEmbeddings(x *ts.Tensor) *ts.Tensor {
	...
	positionIds := ts.MustArangeStart(ts.IntScalar(re.paddingIndex+1), ts.IntScalar(inputShape[0]), torch.Int64, x.MustDevice())
...
func (re *RobertaEmbeddings) ForwardT(inputIds, tokenTypeIds, positionIds, inputEmbeds *ts.Tensor, train bool) (*ts.Tensor, error) {
	...
	if tokenTypeIds.MustDefined() {
		...
		tokTypeIds = ts.MustZeros(inputShape, torch.Int64, inputEmbeddings.MustDevice())
	}
