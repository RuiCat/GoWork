...
func (b *BertModel) ForwardT(inputIds, mask, tokenTypeIds, positionIds, inputEmbeds, encoderHiddenStates, encoderMask *ts.Tensor, train bool) (retVal1, retVal2 *ts.Tensor, retValOpt1, retValOpt2 []ts.Tensor, err error) {
	...
	if mask.MustDefined() {
		...
		maskTs = ts.MustOnes(inputShape, torch.Int64, device)
	}
	...
	case 2:
		if b.IsDecoder {
			seqIds := ts.MustArange(ts.IntScalar(inputShape[1]), torch.Float, device)
	...
	if b.IsDecoder && encoderHiddenStates.MustDefined() {
		...
		if encoderMask.MustDefined() {
			...
			encoderMaskTs = ts.MustOnes([]int64{size[0], size[1]}, torch.Int64, device)
		}
...
func (mlm *BertForMaskedLM) Load(modelNameOrPath string, config interface{ pretrained.Config }, params map[string]interface{}, device torch.Device) error {
