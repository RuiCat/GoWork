...
// the middle.
type TransformStack[T mat.Float]  []mat.Mat4[T]

// NewTransformStack returns a matrix stack where the top element is the
// identity.
func NewTransformStack[T mat.Float] () *TransformStack[T] {
	ms := make(TransformStack[T], 1)
	ms[0] = mat.Ident4[T]()
...
// stack.
func (ms *TransformStack[T]) Push(m mat.Mat4[T]) {
	prev := (*ms)[len(*ms)-1]
...
func (ms *TransformStack[T]) Pop() (mat.Mat4[T], error) {
	if len(*ms) == 1 {
		return mat.Mat4[T]{}, errors.New("attempt to pop last element of the stack; Matrix Stack must have at least one element")
	}
...
// removing it.
func (ms *TransformStack[T]) Peek() mat.Mat4[T] {
	return (*ms)[len(*ms)-1]
}

// Len returns the size of the matrix stack. This value will never be less
// than 1.
func (ms *TransformStack[T]) Len() int {
	return len(*ms)
}
...
// error.
func (ms *TransformStack[T]) Unwind(n int) error {
	if n > len(*ms)-1 {
...
func (ms *TransformStack[T]) Copy() *TransformStack[T] {
	v := append(TransformStack[T]{}, (*ms)...)
	return &v
}
...
func (ms *TransformStack[T]) Reseed(n int, change mat.Mat4[T]) error {
	if n >= len(*ms) || n <= 0 {
...
func (ms *TransformStack[T]) reseed(n int, change mat.Mat4[T]) error {
	backup := []mat.Mat4[T]((*ms)[n:])
	backup = append([]mat.Mat4[T]{}, backup...) // copy into new slice
	...
	for i := n + 1; i < len(*ms); i++ {
		...
		if inv == blank {
			...
			return NoInverseError[T]{Loc: i - 1, Mat: curr}
		}
...
func (ms *TransformStack[T]) undoRebase(n int, prev []mat.Mat4[T]) {
	for i := n; i < len(*ms); i++ {
		(*ms)[i] = prev[i-n]
	}
}
...
func Rebase[T mat.Float] (ms *TransformStack[T], from int, m *TransformStack[T]) (*TransformStack[T], error) {
...
type NoInverseError[T mat.Float]  struct {
	Mat mat.Mat4[T]
	Loc int
}

func (nie NoInverseError[T]) Error() string {
