...
func hostModuleBuilder(r wazero.Runtime) (builder wazero.HostModuleBuilder) {
	builder = r.NewHostModuleBuilder("gojs")
	hfExporter := builder.(wasm.HostFuncExporter)
	pfExporter := builder.(wasm.ProxyFuncExporter)

	pfExporter.ExportProxyFunc(spfunc.MustCallFromSP(false, &wasm.HostFunc{
		Name:        "Str",
		ExportNames: []string{"Str"},
		ParamNames:  []string{},
		...
		Code: &wasm.Code{
			...
			GoFunc: api.GoModuleFunc(func(ctx context.Context, mod api.Module, stack []uint64) {
				buf, _ := mod.Memory().Read(ctx, uint32(stack[0]), uint32(stack[1]))
