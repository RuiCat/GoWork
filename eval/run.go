...
func callOperator(n *node) {
	...
	n.exec = func(f *frame) bltn {
		in := valuefn(f)
		if nod, ok := in.Interface().(*node); ok {
			nod.val = &nod
			nod.recv = &receiver{}
			fr := f.clone(!fork)
			nf := newFrame(fr, len(nod.types), fr.runid())
			for i, t := range nod.types {
				nf.data[i] = reflect.New(t).Elem()
			}
			nf.data[1].Set(value0(f))
			nf.data[2].Set(value1(f))
			runCfg(nod.child[3].start, nf, nod, nod)
			// 处理返回值
			f.data[ni].Set(nf.data[0])
		} else {
			in = reflect.NewAt(in.Type(), unsafe.Pointer(value0(f).UnsafeAddr()))
			fn := in.MethodByName(nv.ident)
			f.data[ni] = fn.Call([]reflect.Value{value1(f)})[0]
		}
		return next
	}
}
