...
func init() { Symbols["eval"]["Symbols"] = reflect.ValueOf(Symbols) }
...
func (interp *Interpreter) AddOperator(name string, rtype Operator) {
	ref := reflect.TypeOf(rtype).Elem()
	it := valueTOf(ref)
	it.str = name
	it.name = name
	interp.scopes["main"].sym[name] = &symbol{kind: typeSym, typ: it}

	fieldCount := ref.NumMethod()
	it.method = make([]*node, fieldCount)
	for i := 0; i < fieldCount; i++ {
		met := ref.Method(i)
		it.method[i] = &node{ident: met.Name, typ: valueTOf(met.Type)}
	}
