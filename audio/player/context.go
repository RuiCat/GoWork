...
// Context is the main object in player. It interacts with the audio drivers.
//
// To play sound with player, first create a context. Then use the context to create
...
func NewContext(options *NewContextOptions) (*Context, chan struct{}, error) {
	...
	if contextCreated {
		return nil, nil, fmt.Errorf("player: context is already created")
	}
	contextCreated = true
