...
var (
	errDeviceSwitched     = errors.New("player: device switched")
	errFormatNotSupported = errors.New("player: the specified format is not supported (there is the closest format instead)")
)
...
func (c *wasapiContext) loopOnRenderThread() error {
	...
	for {
		...
		if evt != windows.WAIT_OBJECT_0 {
			return fmt.Errorf("player: WaitForSingleObject failed: returned value: %d", evt)
		}
