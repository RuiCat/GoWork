// Copyright 2021 The player Authors
//
...
func waveOutOpen(f *_WAVEFORMATEX, callback uintptr) (uintptr, error) {
	...
	if _MMRESULT(r) != _MMSYSERR_NOERROR {
		if e != nil && e != windows.ERROR_SUCCESS {
			return 0, fmt.Errorf("player: waveOutOpen failed: %w", e)
		}
		return 0, fmt.Errorf("player: waveOutOpen failed: %w", _MMRESULT(r))
	}
	return w, nil
}
...
func waveOutClose(hwo uintptr) error {
	...
	if _MMRESULT(r) != _MMSYSERR_NOERROR {
		if e != nil && e != windows.ERROR_SUCCESS {
			return fmt.Errorf("player: waveOutClose failed: %w", e)
		}
		return fmt.Errorf("player: waveOutClose failed: %w", _MMRESULT(r))
	}
	return nil
}
...
func waveOutPrepareHeader(hwo uintptr, pwh *_WAVEHDR) error {
	...
	if _MMRESULT(r) != _MMSYSERR_NOERROR {
		if e != nil && e != windows.ERROR_SUCCESS {
			return fmt.Errorf("player: waveOutPrepareHeader failed: %w", e)
		}
		return fmt.Errorf("player: waveOutPrepareHeader failed: %w", _MMRESULT(r))
	}
	return nil
}
...
func waveOutUnprepareHeader(hwo uintptr, pwh *_WAVEHDR) error {
	...
	if _MMRESULT(r) != _MMSYSERR_NOERROR {
		if e != nil && e != windows.ERROR_SUCCESS {
			return fmt.Errorf("player: waveOutUnprepareHeader failed: %w", e)
		}
		return fmt.Errorf("player: waveOutUnprepareHeader failed: %w", _MMRESULT(r))
	}
	return nil
}
...
func waveOutWrite(hwo uintptr, pwh *_WAVEHDR) error {
	...
	if _MMRESULT(r) != _MMSYSERR_NOERROR {
		if e != nil && e != windows.ERROR_SUCCESS {
			return fmt.Errorf("player: waveOutWrite failed: %w", e)
		}
		return fmt.Errorf("player: waveOutWrite failed: %w", _MMRESULT(r))
	}
	return nil
}
