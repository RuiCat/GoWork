...
func alsaError(name string, err C.int) error {
	return fmt.Errorf("player: ALSA error at %s: %s", name, C.GoString(C.snd_strerror(err)))
}
...
func newContext(sampleRate int, channelCount int, format mux.Format, bufferSizeInBytes int) (*context, chan struct{}, error) {
	...
	go func() {
		...
		if !found {
			...
			c.err.TryStore(fmt.Errorf("player: ALSA error at snd_pcm_open: %s", strings.Join(msgs, ", ")))
			return
		}
