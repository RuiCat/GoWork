// Copyright 2022 The player Authors
//
...
func _CoCreateInstance(rclsid *windows.GUID, pUnkOuter unsafe.Pointer, dwClsContext uint32, riid *windows.GUID) (unsafe.Pointer, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		return nil, fmt.Errorf("player: CoCreateInstance failed: HRESULT(%d)", uint32(r))
	}
	return v, nil
}
...
func (i *_IAudioClient2) GetBufferSize() (uint32, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return 0, fmt.Errorf("player: IAudioClient2::GetBufferSize failed: %w", _AUDCLNT_ERR(r))
		}
		return 0, fmt.Errorf("player: IAudioClient2::GetBufferSize failed: HRESULT(%d)", uint32(r))
	}
...
func (i *_IAudioClient2) GetCurrentPadding() (uint32, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return 0, fmt.Errorf("player: IAudioClient2::GetCurrentPadding failed: %w", _AUDCLNT_ERR(r))
		}
		return 0, fmt.Errorf("player: IAudioClient2::GetCurrentPadding failed: HRESULT(%d)", uint32(r))
	}
...
func (i *_IAudioClient2) GetDevicePeriod() (_REFERENCE_TIME, _REFERENCE_TIME, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return 0, 0, fmt.Errorf("player: IAudioClient2::GetDevicePeriod failed: %w", _AUDCLNT_ERR(r))
		}
		return 0, 0, fmt.Errorf("player: IAudioClient2::GetDevicePeriod failed: HRESULT(%d)", uint32(r))
	}
...
func (i *_IAudioClient2) GetService(riid *windows.GUID) (unsafe.Pointer, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return nil, fmt.Errorf("player: IAudioClient2::GetService failed: %w", _AUDCLNT_ERR(r))
		}
		return nil, fmt.Errorf("player: IAudioClient2::GetService failed: HRESULT(%d)", uint32(r))
	}
	return v, nil
}
...
func (i *_IAudioClient2) Initialize(shareMode _AUDCLNT_SHAREMODE, streamFlags uint32, hnsBufferDuration _REFERENCE_TIME, hnsPeriodicity _REFERENCE_TIME, pFormat *_WAVEFORMATEXTENSIBLE, audioSessionGuid *windows.GUID) error {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return fmt.Errorf("player: IAudioClient2::Initialize failed: %w", _AUDCLNT_ERR(r))
		}
		return fmt.Errorf("player: IAudioClient2::Initialize failed: HRESULT(%d)", uint32(r))
	}
	return nil
}
...
func (i *_IAudioClient2) IsFormatSupported(shareMode _AUDCLNT_SHAREMODE, pFormat *_WAVEFORMATEXTENSIBLE) (*_WAVEFORMATEXTENSIBLE, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		...
		if isAudclntErr(uint32(r)) {
			return nil, fmt.Errorf("player: IAudioClient2::IsFormatSupported failed: %w", _AUDCLNT_ERR(r))
		}
		return nil, fmt.Errorf("player: IAudioClient2::IsFormatSupported failed: HRESULT(%d)", uint32(r))
	}
	return nil, nil
}
...
func (i *_IAudioClient2) SetClientProperties(pProperties *_AudioClientProperties) error {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return fmt.Errorf("player: IAudioClient2::SetClientProperties failed: %w", _AUDCLNT_ERR(r))
		}
		return fmt.Errorf("player: IAudioClient2::SetClientProperties failed: HRESULT(%d)", uint32(r))
	}
	return nil
}
...
func (i *_IAudioClient2) SetEventHandle(eventHandle windows.Handle) error {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return fmt.Errorf("player: IAudioClient2::SetEventHandle failed: %w", _AUDCLNT_ERR(r))
		}
		return fmt.Errorf("player: IAudioClient2::SetEventHandle failed: HRESULT(%d)", uint32(r))
	}
	return nil
}
...
func (i *_IAudioClient2) Start() error {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return fmt.Errorf("player: IAudioClient2::Start failed: %w", _AUDCLNT_ERR(r))
		}
		return fmt.Errorf("player: IAudioClient2::Start failed: HRESULT(%d)", uint32(r))
	}
	return nil
}
...
func (i *_IAudioClient2) Stop() (bool, error) {
	...
	if uint32(r) != uint32(windows.S_OK) && uint32(r) != uint32(windows.S_FALSE) {
		if isAudclntErr(uint32(r)) {
			return false, fmt.Errorf("player: IAudioClient2::Stop failed: %w", _AUDCLNT_ERR(r))
		}
		return false, fmt.Errorf("player: IAudioClient2::Stop failed: HRESULT(%d)", uint32(r))
	}
...
func (i *_IAudioRenderClient) GetBuffer(numFramesRequested uint32) (*byte, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return nil, fmt.Errorf("player: IAudioRenderClient::GetBuffer failed: %w", _AUDCLNT_ERR(r))
		}
		return nil, fmt.Errorf("player: IAudioRenderClient::GetBuffer failed: HRESULT(%d)", uint32(r))
	}
	return data, nil
}
...
func (i *_IAudioRenderClient) ReleaseBuffer(numFramesWritten uint32, dwFlags uint32) error {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isAudclntErr(uint32(r)) {
			return fmt.Errorf("player: IAudioRenderClient::ReleaseBuffer failed: %w", _AUDCLNT_ERR(r))
		}
		return fmt.Errorf("player: IAudioRenderClient::ReleaseBuffer failed: HRESULT(%d)", uint32(r))
	}
	return nil
}
...
func (i *_IMMDevice) Activate(iid *windows.GUID, dwClsCtx uint32, pActivationParams *_PROPVARIANT) (unsafe.Pointer, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		return nil, fmt.Errorf("player: IMMDevice::Activate failed: HRESULT(%d)", uint32(r))
	}
	return v, nil
}
...
func (i *_IMMDevice) GetId() (string, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		return "", fmt.Errorf("player: IMMDevice::GetId failed: HRESULT(%d)", uint32(r))
	}
...
func (i *_IMMDeviceEnumerator) GetDefaultAudioEndPoint(dataFlow _EDataFlow, role _ERole) (*_IMMDevice, error) {
	...
	if uint32(r) != uint32(windows.S_OK) {
		if isWin32Err(uint32(r)) {
			return nil, fmt.Errorf("player: IMMDeviceEnumerator::GetDefaultAudioEndPoint failed: %w", _E_NOTFOUND)
		}
		return nil, fmt.Errorf("player: IMMDeviceEnumerator::GetDefaultAudioEndPoint failed: HRESULT(%d)", uint32(r))
	}
	return endPoint, nil
}
